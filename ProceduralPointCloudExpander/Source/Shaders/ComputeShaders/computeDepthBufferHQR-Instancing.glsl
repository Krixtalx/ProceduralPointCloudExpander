...
layout (std430, binding = 2) buffer InstancesBuffer { mat4		instances[]; };
...
void main()
{
	...
	const uint instanceIndex = id/numPoints;
	if(instanceIndex >= numInstances) return;

	const uint index = (uint)mod(id, numPoints);
	if (index >= numPoints) return;


	// Projection: 3D to 2D
	vec4 projectedPoint	= cameraMatrix * instances[instanceIndex] * vec4(points[index].point, 1.0f);
